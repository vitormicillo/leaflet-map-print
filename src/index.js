...
L.Control.mapPrint = L.Control.extend({
	...
	onAdd: function () {
		...
		const container = L.DomUtil.create('div', 'leaflet-control-mapPrint leaflet-bar leaflet-control');
		if (!this.options.hidden) {
			...
			let btnClass = 'leaflet-control-mapPrint-button';
			if (this.options.exportOnly) btnClass = btnClass + '-export';

			this.link = L.DomUtil.create('a', btnClass, container);
			this.link.id = 'leafletmapPrint';
			this.link.title = this.options.title;
			this.holder = L.DomUtil.create('ul', 'mapPrintHolder', container);

			this.options.sizeModes.forEach(function (sizeMode) {
				const btn = L.DomUtil.create('li', 'mapPrintSizeMode', this.holder);
	...
	printMap: function (event, filename) {
		...
		this._map.fire('mapPrint-start', {event: event});
		if (!this.options.hidden) {
		...
		const sizeMode = typeof event !== 'string' ? event.target.className : event;
	...
	_resizeAndPrintMap: function (sizeMode) {
		...
		this.orientation = this.mapContainer.style.width > this.mapContainer.style.height ? 'portrait' : 'landscape';
	...
	_printOpertion: function (sizemode) {
		...
		domtoimage
			...
			.then(function (dataUrl) {
				...
				plugin._map.fire('mapPrint-finished');
			})
			.catch(function (error) {
	...
	_addCss: function () {
		...
		css.type = 'text/css';
		css.innerHTML = `.leaflet-control-mapPrint-button { 
		...
		.leaflet-control-mapPrint-button-export { 
		...
		.mapPrintHolder .A4Landscape { 
		  transform: rotate(-90deg);
		}
  
		.leaflet-control-mapPrint-button{
		...
		.mapPrintHolder .mapPrintSizeMode a {
		  border-radius: 0px;
		}
  
		.mapPrintHolder .mapPrintSizeMode:last-child a{
		  border-top-right-radius: 2px;
		  border-bottom-right-radius: 2px;
		  margin-left: -1px;
		}
  
		.mapPrintPortrait:hover, .mapPrintLandscape:hover{
